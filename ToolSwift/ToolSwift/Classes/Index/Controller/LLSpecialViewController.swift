...
class LLSpecialViewController: LLBaseViewController {
    ...
    private lazy var tableView: UITableView = {
        ...
        tw.register(cellType: LLSpecialTCell.self)
    ...
    @objc private func loadData(_ showloading: Bool = false, more: Bool) {
        page = (more ? ( page + 1) : 1)
        ApiToolProvider(showloading).request(ApiTool.special(argCon: argCon, page: page), model: ComicListModel.self) { [weak self] (returnData) in
...
extension LLSpecialViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return specialList.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(for: indexPath, cellType: LLSpecialTCell.self)
