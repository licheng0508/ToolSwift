...
class LLReadViewController: LLBaseViewController {
    ...
    lazy var bottomBar: LLReadBottomBar = {
        let br = LLReadBottomBar()
    ...
    func loadData(with index: Int, isPreious: Bool, needClear: Bool, finished: ((_ finished: Bool) -> Void)? = nil) {
        ...
        if index <= -1 {
            collectionView.MJHead.endRefreshing()
            LLNoticeBar(config: UNoticeBarConfig(title: "亲,这已经是第一页了")).show(duration: 2)
        } else if index >= detailStatic.chapter_list?.count ?? 0 {
            collectionView.MJFoot.endRefreshingWithNoMoreData()
            LLNoticeBar(config: UNoticeBarConfig(title: "亲,已经木有了")).show(duration: 2)
        } else {
            guard let chapterId = detailStatic.chapter_list?[index].chapter_id else { return }
            ApiToolProvider(true).request(ApiTool.chapter(chapter_id: chapterId), model: ChapterModel.self) { (returnData) in
                
                self.collectionView.MJHead.endRefreshing()
                self.collectionView.MJFoot.endRefreshing()
...
extension LLReadViewController:UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(for: indexPath, cellType: LLReadCCell.self)
