...
class LLSearchViewController: LLBaseViewController {
    ...
    private lazy var historyTableView: UITableView = {
        ...
        tw.delegate = self
        tw.dataSource = self
        tw.register(headerFooterViewType: LLSearchTHeadView.self)
        tw.register(cellType: LLBaseTableViewCell.self)
        tw.register(headerFooterViewType: LLSearchTFootView.self)
        return tw
    }()
    ...
    lazy var searchTableView: UITableView = {
        ...
        sw.delegate = self
        sw.dataSource = self
        sw.register(headerFooterViewType: LLSearchTHeadView.self)
        sw.register(cellType: LLBaseTableViewCell.self)
        return sw
    }()
    ...
    lazy var resultTableView: UITableView = {
        ...
        rw.delegate = self
        rw.dataSource = self
        rw.register(cellType: LLComicTCell.self)
        return rw
    }()
    ...
    private func loadHistory() {
        ...
        ApiToolProvider().request(ApiTool.searchHot, model: HotItemsModel.self) { (returnData) in
    ...
    private func searchRelative(_ text: String) {
        if text.count > 0 {
            ...
            currentRequest = ApiToolProvider().request(ApiTool.searchRelative(inputText: text), model: [SearchItemModel].self) { (returnData) in
    ...
    private func searchResult(_ text: String) {
        if text.count > 0 {
            ...
            searchBar.text = text
            ApiToolProvider().request(ApiTool.searchResult(argCon: 0, q: text), model: SearchResultModel.self) { (returnData) in
...
extension LLSearchViewController: UITextFieldDelegate {
...
extension LLSearchViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if tableView == historyTableView {
            let head = tableView.dequeueReusableHeaderFooterView(LLSearchTHeadView.self)
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        if tableView == historyTableView && section == 1 {
            let foot = tableView.dequeueReusableHeaderFooterView(LLSearchTFootView.self)
            foot?.data = hotItems ?? []
            foot?.didSelectIndexClosure{ [weak self] (index, model) in
                let vc = LLComicViewController(comicid: model.comic_id)
